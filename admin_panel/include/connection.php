...
$fetch=mysqli_query($con,"select * from `setting` where id = '1'");
	$link=mysqli_fetch_array($fetch);
	$colour = $link['colour'];
...
return $walletResult["$field"];
	...
	function winnerusdt($con,$periodid,$tab,$column)
{
...
FROM
    `tbl_betting` where `periodid`='$periodid' and `tab`='$tab' and `curr`='usdt'");
...
FROM
    `tbl_betting` where `periodid`='$periodid' and `tab`='$tab' and `curr`='trx'");
$result=mysqli_fetch_array($query);	
return $result["$column"];
...
{
$selectquery=mysqli_query($a,"select * from `tbl_betting` where `periodid`='$periodid' and `tab`='$tab' and `curr`='trx' and `value`in($value) group by `userid`
");
