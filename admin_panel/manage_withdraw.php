...
$sqlwallet= mysqli_query($con,"UPDATE `tbl_wallet` SET `amount` = '$finalbalanceCredit' WHERE `userid`= '$userid'");	
 ...
 ?> 
  ...
  $total+=$row['amount'];?>  
              ...
              <td><?php echo strtoupper($row['payout']); ?> <?php  $amount = $row['amount'];
                           $total1 = $amount*1/100;
                           $final = $amount-$total1;
                      
                         echo round($final,2);?></td>
      ...
      <td><button  class="btn copyBtn" data-clipboard-text="<?php echo @$row["address"]; ?>">Copy</button></td>
			     <td>  <div class="col-md-12">
                      <?php if($row['status']==0){?>
						...
						<?php }else if($row['status']==1){?><div class="col-md-6 "><a href="javascript:void(0);" class="btn btn-success btn-block" onClick="acceptrejectconfirm(<?php echo $row['id'];?>,'accept');"><strong><i class="fa fa-check"></i> Accept</strong></a> </div>
                        <div class="col-md-6 "> <a href="javascript:void(0);" class="btn btn-danger btn-block" onClick="acceptrejectconfirm(<?php echo $row['id'];?>,'reject');"><strong><i class="fa fa-times"></i> Reject</strong></a> </div>
                        <?php }?>
                      </div></td>
