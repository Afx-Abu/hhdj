...
<html>
   ...
   <body class="hold-transition skin-yellow-light sidebar-mini">
                              ...
                              <table id="example1" class="table table-striped text-center">
                                 <h4 class="table_bg ">
                                    ...
                                    <span class="pull-right">
                                       Count Down: 
                                       <spam id="demo" class="red_txt "></spam>
                                       || Game ID: <?php
    ...
    $result = mysqli_query($con,"SELECT 
        value, 
        count(userid) as 'all_users', 
        SUM(CASE WHEN curr = 'trx' THEN amount ELSE 0 END) as 'sum_trx_amount', 
        SUM(CASE WHEN curr = 'trx' THEN amount ELSE 0 END) as 'sum_trx_amount' 
        FROM tbl_betting 
        WHERE periodid = $cr_gameid 
        GROUP BY value");
    ...
    if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_array($result)) {
            ...
            $sum_trx_amount = $row["sum_trx_amount"];
            $sum_usdt_amount = $row["sum_usdt_amount"];

            // Perform the multiplication
            $total_amount_get = $sum_usdt_amount * 11.77;
             $total_amount = $total_amount_get + $sum_trx_amount;
            // Display the results
            echo "<tr>";
            echo "<td>$value</td>";
            echo "<td>$all_users</td>";
            echo "<td>TRX " . number_format($total_amount,2) . "</td>";
            echo "</tr>";
        }
    }
?>
                                       ...
                                       <td>		    <?php
                                          $totalRecharge['total'] = 0;
                                                                                    ...
                                                                                    ?></td>
                                                                                     ...
                                                                                     <td>		    <?php
                                          $totalRecharge['total'] = 0;
                                                                                        $q = mysqli_query($con,"SELECT amountusdt as total FROM tbl_wallet WHERE userid = '".$row["userid"]."' ");
                                                                                        $totalRecharge = mysqli_fetch_assoc($q);
                                                                                        echo round($totalRecharge['total'],2);
