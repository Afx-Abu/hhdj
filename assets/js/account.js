...
document.getElementById('regtoast').innerHTML = ('<font >Mobile Number allready register with us !</font>');
			$('#registertoast').modal({backdrop: 'static', keyboard: false})  
                   $('#registertoast').modal('show');
			}
			else if (arr[0]== 3) {
document.getElementById('regtoast').innerHTML = ('<font >Wrong recommendation code enter !</font>');
			$('#registertoast').modal({backdrop: 'static', keyboard: false})  
                   $('#registertoast').modal('show');
			}
else if (arr[0]== 4) {
document.getElementById('regtoast').innerHTML = ('<font >Please verify your mobile no. !</font>');
			$('#registertoast').modal({backdrop: 'static', keyboard: false})  
                   $('#registertoast').modal('show');
			}
			else if(arr[0]==0)
			{ document.getElementById('serror').innerHTML = ('<font >Some Technical error !</font>');
				}
...
if (loginpassword.length<5) {
		...
		$.ajax({
			...
			{ document.getElementById('regtoast').innerHTML = ('<font >Wrong Login Detail Entered!</font>');
			$('#registertoast').modal({backdrop: 'static', keyboard: false})  
                   $('#registertoast').modal('show');
				}
...
if ((otp)== "") {
		...
		$.ajax({
			...
			if (arr[0]== 1) {
			$("#otpclose").click();	
	document.getElementById('regtoast').innerHTML = ('<font size="2" style="color:#fff;">OTP match successfully !</font>');
			$('#registertoast').modal({backdrop: 'static', keyboard: false})  
            $('#registertoast').modal('show');
			$('#mobile').attr('readonly', true);
			}	
			else if(arr[0]==0)
			{ document.getElementById('regtoast').innerHTML = ('<font >Wrong OTP Entered !</font>');
			$('#registertoast').modal({backdrop: 'static', keyboard: false})  
            $('#registertoast').modal('show');
				}
