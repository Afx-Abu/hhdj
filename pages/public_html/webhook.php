...
$nowpayment=mysqli_query($con,"select * from `nowpayment` where `amount_status` = '0'");
...
$userlevel1Rows=mysqli_fetch_array($userlevel1); $userreferfineded = $userlevel1Rows['id'];
  ...
  if ($status === "finished") {
    if ($wallet_status == '0') {
      ...
      $sql= mysqli_query($con,"UPDATE `nowpayment` SET `payment_status` = 'finished', `amount_status` = '1' where `payment_id` = '$payment_id'");
		...
		if($chkorderRow==''){
			...
			$sql3= mysqli_query($con,"INSERT INTO `tbl_walletsummery`(`userid`,`orderid`,`amount`,`type`,`actiontype`,`createdate`) VALUES ('".$userid."','".$id."','".$amount."','credit','recharge','".$today."')");
			  ...
			  if($rechargeCount == 0){
			 $walletSql = mysqli_query($con,"UPDATE `tbl_wallet` SET `amount`= `amount`+'".$bamount2."' WHERE userid = $userreferfineded");
