...
<head>
  ...
  <title>Admin Panel | Manage Recharge History</title>
...
<h1>Manage Recharge Rejected</h1>
      <ol class="breadcrumb">
           
        <li><a href="desktop.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Manage Recharge Rejected</li>
      </ol>
    </section>
...
//  $Query=mysqli_query($con,"select *,(select `mobile` from `tbl_user` where `id`=`tbl_withdrawal`.`userid`)as user from `tbl_withdrawal` where `status`='1' order by id desc");
 $Query=mysqli_query($con,"select * from `tbl_recharge` where `status`='0' order by id desc ");
  $i=0;
 ...
 $convert=date('h:i A d-m-Y',$post_date2);
     $sqlget = mysqli_query($con,"SELECT * FROM tbl_user WHERE id  =  $i");
     $sqlgetresult =mysqli_fetch_array($sqlget);
     $mobilenew =    $sqlgetresult['mobile'] ;
            ...
            $mobilenew = $sqlgetresult['mobile'];  echo $mobilenew;             ?></td>
              <td><?php echo number_format($row['amount']);?></td>
               ...
               <td><?php echo strtoupper(@$row["txn"]); ?></td>
                     <!--  <td><a href="https://wingoplus.com/alexa/uploads/<?php echo @$row["file_name"]; ?>" target="_blank"><img src="uploads/<?php echo @$row["file_name"]; ?>" height="50px" alt="<?php echo strtoupper(@$row["file_name"]); ?>"></a></td>-->
